...
pub fn body (v:&Value) -> (String,String){
  let page_width = &v["page-width"].as_str().unwrap_or(setting::DEFAULT_PAGE_WIDTH);
  let page_height = &v["page-height"].as_str().unwrap_or(setting::DEFAULT_PAGE_HEIGHT);
  let page_size = &v["page-size"].as_str();

  let main_font_size = &v["main-font"]["font-size"].as_str().unwrap_or(setting::DEFAULT_FONT_SIZE);
  ...
  let main_font_correction_latin = &v["main-font"]["latin-correction"].as_str().unwrap_or("hoge");


  let top_space = &v["top-space"].as_str().unwrap_or(setting::DEFAULT_TOP_SPACE);
  let bottom_space = &v["bottom-space"].as_str().unwrap_or(setting::DEFAULT_BOTTOM_SPACE);
  let left_space = &v["left-space"].as_str().unwrap_or(setting::DEFAULT_LEFT_SPACE);
  let right_space = &v["right-space"].as_str().unwrap_or(setting::DEFAULT_RIGHT_SPACE);

  let font_data = &v["font-data"].as_array();


  let document_function_name = &v["doc-fun-name"].as_str().unwrap_or(setting::DEFAULT_DOCUMENT_FUNCTION_NAME);
  ...
  let def_value_vec =
    vec![
      setting::set_main_font(&main_font_size,&main_font_name_cjk,&main_font_ratio_cjk,&main_font_correction_cjk,&main_font_name_latin,&main_font_ratio_latin,&main_font_correction_latin),
